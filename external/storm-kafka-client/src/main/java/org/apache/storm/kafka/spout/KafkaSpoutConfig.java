 ...
 */
  ...
  public KafkaSpoutTuplesBuilder<K,V> tuplesBuilder;
  ...
  public Map<String,Object> setDefaultsAndGetKafkaProps(  Map<String,Object> kafkaProps){
...
public static class Builder<K,V> {
    public Map<String,Object> kafkaProps;
    public SerializableDeserializer<K> keyDeserializer;
    public SerializableDeserializer<V> valueDeserializer;
    ...
    public KafkaSpoutRetryService retryService;
    /** 
 * Please refer to javadoc in   {@link #Builder(Map,KafkaSpoutStreams,KafkaSpoutTuplesBuilder,KafkaSpoutRetryService)}.<p/> This constructor uses by the default the following implementation for   {@link KafkaSpoutRetryService}:<p/>  {@code new KafkaSpoutRetryExponentialBackoff(TimeInterval.seconds(0), TimeInterval.milliSeconds(2),DEFAULT_MAX_RETRIES, TimeInterval.seconds(10)))}
 */
    public Builder(    Map<String,Object> kafkaProps,    KafkaSpoutStreams kafkaSpoutStreams,    KafkaSpoutTuplesBuilder<K,V> tuplesBuilder){
      this(kafkaProps,kafkaSpoutStreams,tuplesBuilder,new KafkaSpoutRetryExponentialBackoff(TimeInterval.seconds(0),TimeInterval.milliSeconds(2),DEFAULT_MAX_RETRIES,TimeInterval.seconds(10)));
    }
    /** 
 ...
 */
  ...
  public boolean isConsumerAutoCommitMode(){
    return kafkaProps.get(Consumer.ENABLE_AUTO_COMMIT) == null || Boolean.valueOf((String)kafkaProps.get(Consumer.ENABLE_AUTO_COMMIT));
  }
  public String getConsumerGroupId(){
    return (String)kafkaProps.get(Consumer.GROUP_ID);
  }
  /** 
 * @return list of topics subscribed and emitting tuples to a stream as configured by {@link KafkaSpoutStream}, or null if this stream is associated with a wildcard pattern topic
 */
  public List<String> getSubscribedTopics(){
    return kafkaSpoutStreams instanceof KafkaSpoutStreamsNamedTopics ? new ArrayList<>(((KafkaSpoutStreamsNamedTopics)kafkaSpoutStreams).getTopics()) : null;
  }
  /** 
 * @return the wildcard pattern topic associated with this {@link KafkaSpoutStream}, or null if this stream is associated with a specific named topic
 */
  public Pattern getTopicWildcardPattern(){
    return kafkaSpoutStreams instanceof KafkaSpoutStreamsWildcardTopics ? ((KafkaSpoutStreamsWildcardTopics)kafkaSpoutStreams).getTopicWildcardPattern() : null;
  }
  ...
  @Override public String toString(){
    return "KafkaSpoutConfig{" + "kafkaProps=" + kafkaProps + ", keyDeserializer="+ keyDeserializer+ ", valueDeserializer="+ valueDeserializer+ ", pollTimeoutMs="+ pollTimeoutMs+ ", offsetCommitPeriodMs="+ offsetCommitPeriodMs+ ", maxUncommittedOffsets="+ maxUncommittedOffsets+ ", firstPollOffsetStrategy="+ firstPollOffsetStrategy+ ", kafkaSpoutStreams="+ kafkaSpoutStreams+ ", tuplesBuilder="+ tuplesBuilder+ ", retryService="+ retryService+ ", topics="+ getSubscribedTopics()+ ", topicWildcardPattern="+ getTopicWildcardPattern()+ '}';
  }
